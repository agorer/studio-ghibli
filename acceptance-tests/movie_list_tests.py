...
class MovieListTests(unittest.TestCase):
    ...
    def test_should_contain_a_list_of_movies(self):
        ...
        for movie in movies:
            title = movie.find_elements_by_class_name('title')
            self.assertTrue(title.strip())

            description = movie.find_elements_by_class_name('description')
            self.assertTrue(description.strip())

            release_date = movie.find_elements_by_class_name('release-date')
            self.assertTrue(release_date.strip().isdigit())
